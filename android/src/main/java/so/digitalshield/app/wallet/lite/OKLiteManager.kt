...
import so.digitalshield.app.wallet.lite.nfc.NFCExceptions
import so.digitalshield.app.wallet.lite.nfc.NfcUtils
import so.digitalshield.app.wallet.lite.nfc.broadcast.NfcStatusChangeBroadcastReceiver
import so.digitalshield.app.wallet.lite.digitalshieldLite.DigitalShieldLiteCard
import so.digitalshield.app.wallet.lite.digitalshieldLite.entitys.CardState
import so.digitalshield.app.wallet.lite.utils.NfcPermissionUtils
import so.digitalshield.app.wallet.lite.utils.Utils
...
//                            mNFCConnectedChannel.receive()
                            val startRequest = DigitalShieldLiteCard.initRequest(isoDep)
                            val dataMap = Arguments.createMap().apply {
                                putInt("code", -1)
                                putString("type", "DigitalShield_Lite")
    ...
    override fun initialize() {
        ...
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                DigitalShieldLiteCard.startNfc(it as FragmentActivity) {}
            }
        }
    ...
    override fun onHostResume() {
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                ...
                try {
                    DigitalShieldLiteCard.startNfc(it) {
    ...
    override fun onHostPause() {
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                try {
                    DigitalShieldLiteCard.stopNfc(it as FragmentActivity)
    ...
    private suspend fun acquireDevice(): IsoDep? {
        ...
        if (receiveIsoDep == null) {
            ...
            val initChannelRequest = DigitalShieldLiteCard.initRequest(receiveIsoDep)
    ...
    fun getLiteInfo(callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            val cardInfo = DigitalShieldLiteCard.getCardInfo(isoDep)
    ...
    fun setMnemonic(mnemonic: String, pwd: String, overwrite: Boolean, callback: Callback) =
        launch {
            handleOperation(callback) { isoDep ->
                ...
                val isSuccess =
                    DigitalShieldLiteCard.setMnemonic(mCurrentCardState, isoDep, mnemonic, pwd, overwrite)
    ...
    fun getMnemonicWithPin(pwd: String, callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            DigitalShieldLiteCard.getMnemonicWithPin(mCurrentCardState, isoDep, pwd)
        }
    }
    ...
    fun changePin(oldPwd: String, newPwd: String, callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            DigitalShieldLiteCard.changPin(mCurrentCardState, isoDep, oldPwd, newPwd)
        }
    }
    ...
    fun reset(callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            val isSuccess = DigitalShieldLiteCard.reset(isoDep)
            if (!isSuccess) throw NFCExceptions.ExecFailureException()
